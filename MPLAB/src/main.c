...
int status = 0;
uint16_t localTemp;
uint16_t tmpLocalTemp;
uint16_t threshold = 125;
;
uint16_t tempReal = 0;
uint16_t temp1;
uint16_t temp2;
...
void main(void) {
    ...
    startDS18B20()
            ;
    while (1) {
        tmpLocalTemp = getDS18B20();
        if (tmpLocalTemp != 0xFF) {
            localTemp = tmpLocalTemp;
            startDS18B20();
        }
        ...
        workWebServer();
        tempReal = localTemp + temp1;
        if (tempReal > threshold || enable != 1) {
            disableOut();
        } else {
            enableOut();
        }
